...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        let scoreLabelNode = SKLabelNode(fontNamed: "Marker Felt")
        scoreLabelNode.text = "SCORE"
        scoreLabelNode.fontSize = 40
        scoreLabelNode.fontColor = SKColor.white
        scoreLabelNode.position = CGPoint(x: self.frame.size.width/2, y: self.frame.size.height*0.75)
        ...
        karateKidNode.position = CGPoint(x: self.frame.size.width/2, y: self.frame.size.height*0.25)
